...
PointCloudClass::PointCloudClass()
{
	...
	sub_odom=nhSub.subscribe("/mynteye_pointcloud/odom",1,&PointCloudClass::odom_callback,this);
	//sub_encoder=nhSub.subscribe("/encoder",1,&PointCloudClass::encoder_callback,this);
	//publisher
    pubpc= nhPub1.advertise<mynteye_pointcloud::PointCloudData>("Extracted_PointCloud", 1);
	pubpc1= nhPub1.advertise<mynteye_pointcloud::PointCloudData>("Created_PointCloud", 1);
	pub_clus= nhPub1.advertise<mynteye_pointcloud::ClusterData>("Clustering_PointCloud", 1);
	// lanchファイルの読み込み
	setLaunchParam();
