...
//self msg
#include <mynteye_pointcloud/SensorMapData.h>
// #include <mynteye_pointcloud/CompressedSensorData.h>
...
#include <mynteye_pointcloud/classificationConfig.h>
//クラスの定義
class classificationClass{
    private:
        ...
        mynteye_pointcloud::SensorMapData smdCamera, smdLRF;
        //送信データ
		ros::NodeHandle nhPub;
        ros::Publisher pub;
        mynteye_pointcloud::ClassificationData cd;
        //クラスタリング処理
        // std::vector<int> mapIndex;//
    	std::vector<int> winIndex;//基準点（コア点）から参照値
        // mynteye_pointcloud::CompressedSensorData compCamData;
        //--カメラパラメータ
        ...
        dynamic_reconfigure::Server<mynteye_pointcloud::classificationConfig> server;
        dynamic_reconfigure::Server<mynteye_pointcloud::classificationConfig>::CallbackType f;
    public:
        //in constracter.cpp
        ...
        void cameraMap_callback(const mynteye_pointcloud::SensorMapData::ConstPtr& msg);
        void subscribeSensorDataLRF();//データ受信
        void laserMap_callback(const mynteye_pointcloud::SensorMapData::ConstPtr& msg);
        //--rqt_reconfigureからの読み込み
        void configCallback(mynteye_pointcloud::classificationConfig &config, uint32_t level);
        //--クラスタリング
